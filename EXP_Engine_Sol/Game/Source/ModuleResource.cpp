...
void ModuleResource::LoadAssets()
{
...
bool ModuleResource::IterateAssets(PathNode node, uint ID)
{
	bool isNew = true;
	...
	if (App->fileSystem->Exists(metaPath.c_str()))
	{
		...
		if (size > 0)
		{
			...
			std::map<uint, Resource*>::iterator iterator = importedResources.find(jsonMeta.GetNumber("UID"));
			...
			{
				std::string name = jsonMeta.GetString("Name");
				uint UID = jsonMeta.GetNumber("UID");
				...
				if (resource->type == ResourceType::Model || resource->type == ResourceType::Scene)
				{
					...
					for (uint i = 0; i < resourceInModel.GetSize(); i++)
					{
						...
						std::string name = metaInModel.GetString("Name");
						uint UID = metaInModel.GetNumber("UID");
...
uint ModuleResource::Find(const char* file)
{
	return uint();
}

uint ModuleResource::ImportFile(const char* assetsFile)
{
...
void ModuleResource::LoadScene(const char* buffer, uint size, ResourceScene* scene)
{
...
void ModuleResource::ReleaseResource(uint UID)
{
}

Resource* ModuleResource::CreateNewResource(const char* assetsFile, ResourceType type, const char* name, uint32 UID)
{
...
void ModuleResource::SaveMeta(Resource* resource)
{
	string name;
	string extension;
...
void ModuleResource::SaveResource(Resource* resource)
{
	char* buffer = nullptr;
	uint size = 0;
...
Resource* ModuleResource::LoadResource(uint32 UID, Resource* resource)
{
...
Resource* ModuleResource::AccesResource(uint32 UID)
{
	LOG("Loading: %d", UID);
...
ResourceType ModuleResource::GetTypeFromFile(const char* path) const
{
...
ResourceType ModuleResource::GetTypefromString(std::string typeString) const
{
...
std::string ModuleResource::GetStringFromResource(Resource* resource)
{
	std::string path;
